...
def simulate_adaptive_control():
    ...
    for i, t in enumerate(t_span):
        ...
        # Get true dynamics for comparison
        f_true = system.f(x)

        # Collect data for retraining
        controller.collect_data(x, f_true)

        # Update adaptive parameters
        controller.update_weights(e, sigma_hat)
        controller.update_parameters(x, u, t, e)

        # System dynamics integration (simplified Euler method)
        x_dot = system.dynamics(t, x, u)
        x = x + x_dot * dt
        ...
        xd_history.append(controller.desired_trajectory(t))
        W_norm_history.append(np.linalg.norm(controller.W_hat))

        # Check for retraining
        if (current_retrain_idx < len(retrain_times) and
                t >= retrain_times[current_retrain_idx] and
                t <= retrain_completed[current_retrain_idx]):

            if len(controller.data_x) > 100:  # Ensure sufficient data
                print(f"Retraining DNN at t = {t:.1f}s...")
                X_retrain = np.array(controller.data_x[-1000:])  # Use recent data
                y_retrain = np.array(controller.data_f[-1000:])

                # Retrain DNN
                dnn.retrain(X_retrain, y_retrain, epochs=50)

                if t >= retrain_completed[current_retrain_idx]:
                    current_retrain_idx += 1

        # Progress update
        if i % 1000 == 0:
            print(f"Progress: {t:.1f}/{t_final:.1f} seconds")
